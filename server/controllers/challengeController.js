...
exports.createChallenge = async (req, res) => {
    try {
        ...
        // Resolve challenger correctly
        const user = await User.findOne({
            $or: [
                { _id: mongoose.Types.ObjectId.isValid(challengerId) ? challengerId : null },
...
exports.acceptChallenge = async (req, res) => {
    try {
        ...
        const user = await User.findOne({
            $or: [
                { _id: mongoose.Types.ObjectId.isValid(acceptorId) ? acceptorId : null },
                { playerId: acceptorId }
            ]
        });
        if (!user) return res.status(404).json({ success: false, message: 'User not found' });
